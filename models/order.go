...
type Order struct {
	...
	StarPerPrice  int    `json:"star_per_price" form:"-" gorm:"type:int(12);not null;default:0"` //平均每颗星价格
	...
	Title         string `json:"title" form:"title" gorm:"type:varchar(100);not null;default:''"` //订单标题
	...
	Margin        int    `json:"margin" gorm:"type:int(7);not null;default:0"`                                                   //保证金总额
	MarginSafe    int    `json:"margin_safe" form:"margin_safe" gorm:"type:int(7);not null;default:0" valid:"Range(0, 9000000)"` //安全保证金
	MarginEff     int    `json:"margin_eff" form:"margin_eff" gorm:"type:int(7);not null;default:0" valid:"Range(0, 9000000)"`   //效率保证金
	MarginArb     int    `json:"margin_arb" form:"margin_arb" gorm:"type:int(7);not null;default:0" valid:"Range(0, 9000000)"`   //仲裁保证金
	...
	PayAmount     int    `json:"pay_amount" form:"-" gorm:"type:int(12);not null;default:0"` //用户支付的订单费用 与订单价格相同 单位分
	...
	TakerUserId        int    `json:"taker_user_id" form:"-" gorm:"type:int(12);not null;default:0"`    //接单人id
	TakerNickName      string `json:"taker_nick_name" gorm:"type:varchar(32);not null;default:''"`      //接单人name
	TakerPayAmount     int    `json:"taker_pay_amount" form:"-" gorm:"type:int(12);not null;default:0"` //接单人在接单时支付的保证金金额 单位分
	...
	RefundAmount    int    `json:"refund_amount" form:"-" gorm:"type:int(12);not null;default:0"` //退还的保证金金额 单位分
	TakerRefundTime int    `json:"taker_refund_time" gorm:"type:int(12);not null;default:0"`      //如果为0 表示没有退还，大于0表示保证金退还成功

	PayRefundTradeNo string `json:"pay_refund_trade_no" gorm:"type:varchar(50);not null;default:''"`   //用户退款
	PayRefundAmount  int    `json:"pay_refund_amount" form:"-" gorm:"type:int(12);not null;default:0"` //用户退款金额
	PayRefundTime    int    `json:"pay_refund_time" gorm:"type:int(12);not null;default:0"`            //如果为0 表示没有退还，大于0表示用户金额退还成功
	...
	AdjudgeUserId int    `json:"adjudge_user_id" form:"-" gorm:"type:int(12);not null;default:0"` //发起仲裁的用户id
	AdjudgeMsg    string `gorm:"type:varchar(1000);not null;default:''" json:"adjudge_msg"`       //仲裁的留言
	ImgAdjudgeUrl string `gorm:"type:varchar(1200);not null;default:''" json:"img_adjudge_url"`   //仲裁留言中的图片
}
