...
func UrgentPay(userId int, orderId int, ip string) (map[string]interface{}, bool) {
	totalFee := var_const.TeamUrgentPrice
	openId, err := auth_service.GetUserOpenId(userId)
	if err != nil || openId == "" {
		return nil, false
	}
	...
	payReq.MchId = var_const.WXMchID
	payReq.NonceStr = GenerateNonceStr()
	payReq.NotifyUrl = "https://www.bafangwangluo.com/pay/urgentteamnotify"
	payReq.OpenId = openId
	payReq.TradeType = tradeType
	payReq.SpbillCreateIp = ip
	payReq.TotalFee = totalFee
	...
	if strings.ToUpper(resp1.ReturnCode) == "SUCCESS" && strings.ToUpper(resp1.ResultCode) == "SUCCESS" {
		...
		var m = make(map[string]interface{})
		m["urgent_trade_no"] = payOrderId
		m["urgent_trade_no"] = userId
		m["urgent_nick_name"], _ = auth_service.GetUserNickName(userId)
		m["urgent_pay_amount"] = totalFee
